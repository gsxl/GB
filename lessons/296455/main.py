...
def in1():
    ...
    if (check_int(in1) != "" and len(in1) == 1 and int(in1) > 0
            and int(in1) < 8):
        weekday = [
            'бездельник,', 'повторник,', 'бреда,', 'чертегдерг,',
            'расслабильница,', 'клуббота,', 'высплюсенье,'
        ]
        if (int(in1) >= 6):
            w = "и это выходной ;)"
        else:
...
def in4():
    ...
    d3 = (t2x - t1x)**2 + (t2y - t1y)**2 + (t2z - t1z)**2
    d2 = (t2x - t1x)**2 + (t2y - t1y)**2 

    print("Расстояние между точками 3D: ", sqrt(d3))
    print("Расстояние между точками 2D: ", sqrt(d2))
